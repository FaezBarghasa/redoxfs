...
use redoxfs::{DiskFile, FileSystem, TreePtr, Node};
...
fn test_recovery() {
    ...
    // 1. Create a filesystem and write some data
    {
        ...
        fs.lock().tx(|tx| {
            let root_ptr = TreePtr::<Node>::root();
            let a = tx.create_node(root_ptr, "a", Node::MODE_FILE | 0o644, 0, 0)?;
    ...
    // 2. Re-open the filesystem and check for the data
    {
        ...
        let fs = FileSystem::open(disk, None, None, false).unwrap();
